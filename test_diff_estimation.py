...
def get_review_feedback(patch):
    ...
    diff_text = patch
    prompt = open('data/diff_estimation_prompt.txt').read()
...
with open('prompt/diff_estimation.jsonl', 'r') as file:
    patches = [json.loads(line) for line in file]

# 현재 시간 추가
current_time = datetime.now().strftime('%Y%m%d_%H%M%S')
# output.jsonl 파일에 저장
with open('output/diff_estimation_{time}.jsonl', 'w') as output_file:
    try:
        y_true = []
        y_pred = []
        ...
        for patch in tqdm(patches, desc="Processing patches"):
            ...
            review_needed = get_review_feedback(patch['patch'])
            ...
            #print(f"Review needed for patch ID {patch['id']}: {review_needed}")
